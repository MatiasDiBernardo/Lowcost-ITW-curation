...
class ComputeScore:
    ...
    def __call__(self, fpath, sampling_rate, is_personalized_MOS):
        ...
        if aud.ndim > 1:
            aud = np.mean(aud, axis=1)  # Convert to mono by averaging channels
        fs = sampling_rate
        if input_fs != fs:
...
def main(args):
    ...
    clips = clips + (glob.glob(os.path.join(args['testset_dir'], "*.mp3")))
    is_personalized_eval = args['personalized_MOS']
    desired_fs = SAMPLING_RATE
    for m in tqdm(models):
        max_recursion_depth = 10
        audio_path = os.path.join(args['testset_dir'], m)
        wavs = glob.glob(os.path.join(audio_path, "*.wav"))
