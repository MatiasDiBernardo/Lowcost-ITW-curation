...
class nisqaModel(object):
    ...
    def _loadDatasetsFolder(self):
        wavs = glob(os.path.join(self.args['data_dir'], '*.wav') )
        mp3 = glob(os.path.join(self.args['data_dir'], '*.mp3') )
        files = wavs + mp3
    ...
    def _loadModel(self):    
        ...
        if self.args['pretrained_model']:
            # NC: Agrego esto para poder cargar los weights desde ASR-Dataset-Generator/main.py. 
            # (os.getcwd() estaba pensado para usar desde la cmd parado en la carpeta de nisqa)
            nisqa_main_dir = Path(__file__).resolve().parents[1]
            if os.path.isabs(self.args['pretrained_model']):
                # model_path = os.path.join(self.args['pretrained_model'])
                model_path = os.path.join(nisqa_main_dir, self.args['pretrained_model'])
            else:
