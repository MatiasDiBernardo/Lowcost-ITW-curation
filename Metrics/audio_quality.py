...
def calculate_SNR(file_path):
    """Calculate SNR using WADA SNR.

    Args:
        file_path (float): Path audio file
    ...
    acc_signal_energy = 0
    acc_noise_energy = 0
    for block in sf.blocks(file_path, blocksize=100000):
...
def calculate_PESQ_SISDR(file_path, model):
    """Calculate blind PESQ and SISDR with PytorchSquim

    Args:
        file_path (str): Path audio file 
        model (object): Pytorch squim model 

    Returns:
        float, float: PESQ val, SI-SDR val
    """

    WAVEFORM_SPEECH, SAMPLE_RATE_SPEECH = torchaudio.load(file_path)
    ...
    stoi = (float(stoi_hyp[0]))
    pesq = (float(pesq_hyp[0]))
    sisdr = (float(si_sdr_hyp[0]))
    
    return pesq, sisdr

def calculate_audio_quality(folder_path):
    ...
    for folders in tqdm(os.listdir(folder_path)):
        for audio_path in os.listdir(os.path.join(folder_path, folders)):
            ...
            pesq_val, sisdr_val = calculate_PESQ_SISDR(file_name, model)
            snr_val = calculate_SNR(file_name)
            
            SNR.append(snr_val)
            PESQ.append(pesq_val)
            SISDR.append(sisdr_val)
    
    snr_mean = np.mean(SNR)
    pesq_mean = np.mean(PESQ)
    sisdr_mean = np.mean(SISDR)

    return pesq_mean, sisdr_mean, snr_mean
